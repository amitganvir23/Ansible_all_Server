...
#new
rm  /share/backup/test/succ test_email_smh.retry 2> /dev/null
...
for i in `cat $file 2> /dev/null|tr '\n' ' '`;do echo -e "${ap}\t| ${i}\t\t|== Failed" >> /share/backup/test/succ  ;done
...
#--------- send app status
#for i in `cat test_email_smh.retry`;do echo -e "SMH \t| $i \t| == Failed" >> /share/backup/test/succ ;done
#echo "+++++++++++++++++++++++++"
#cat /share/backup/test/succ
